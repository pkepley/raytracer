...
def c(x):
	return (1.0 
			#+ 0.25 * np.sin(np.pi * (x[0] + x[1]))
			- 0.5 * np.exp( -4.0 * ((x[0]-0.5) * (x[0]-0.5) + (x[1] - 0.5) * (x[1] - 0.5)))
			- 0.5 * np.exp( -4.0 * ((x[0]+0.5) * (x[0]+0.5) + (x[1] - 1.0) * (x[1] - 1.0))) )
rt = raytracer(c,2)
...
if __name__ == '__main__':
	fig,ax = plot_background(-2.0, 2.0, 0.0 ,3.0,100,100);	
	data = add_normals_to_plot(fig, ax, 2.0, 31, 100, -2,2);
	add_isochrones_to_plot(fig, ax, data, 100,3);
	fig.savefig('./img/normals_with_isochrones.png', bbox_inches='tight')
	...
	n_rays = 31;
	n_steps = 100;
	theta_lo, theta_hi = np.pi, 2.0 * np.pi
	fig,ax = plot_background(-2.0, 2.0, 0.0 ,3.0,100,100);
	data, xs_central = add_endpt_fan_to_plot(fig, ax, x0, xi0, T, n_rays, n_steps, theta_lo, theta_hi)
	add_isochrones_to_plot(fig, ax, data, n_steps, 3);
	fig.savefig('./img/ray_fan_with_isochrones.png', bbox_inches='tight')
